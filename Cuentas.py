...
class cuenta:
    def __init__(self,id_usuario="",sitio="",url="",login="",password="",master_password=""):
        self.sitio = sitio
    ...
    def crear_cuenta(self):
        ...
        self.sitio,self.login,self.url,self.password = Cifrado(self.master_password).cifrado_suave(self.sitio,self.login,self.url,self.password,self.key)
        try:
    ...
    def verificar_cuentas(self):
        ...
        """
        try:
            self.query = self.conexion.execute("select Nombre_cuenta,Login,URL,contraseña,sal from cuentas where ID_usuario = ?",(self.id_usuario))
